...
#
#==============================================================
# This script contains the main function to import data into POLAAAR.
# It can take a metadata.MIxS, a DwC.event or a DwC.occurrence class object, and parse it into the different
# tables of the backend database of POLAAAR. This function assumes the data has been QC'd, and only needs to be
# paresed.
#
# in this version, the output is still the indivudual tables (later versions should be able to write directly to POLAAAR)
#==============================================================

#' format data to upload to the POLAAAR database
#'
#' @author Maxime Sweetlove ccBY 4.0 2019
#'
#' @description metadata.to.polaaar writes a metadata.MIxS, DwC.event or a DwC.occurrence class object object to the polaaar database
#' @param metadata.object a metadata.MIxS, DwC.event or a DwC.occurrence class object. The object to be formatted to fit the polaaar database schema
...
metadata.to.polaaar <- function(metadata.object, EML.url=NA, user_ID="",
  ...
  if(exists("occurrence")){
    write.csv(Sequences, paste(dest.dir, "/", baseName, "_occurrence.csv", sep=""), na="", row.names = FALSE)
  }
  if(exists("Taxa")){
    write.csv(Sequences, paste(dest.dir, "/", baseName, "_Taxa.csv", sep=""), na="", row.names = FALSE)
  }
  if(exists("sampling_method")){
    write.csv(Sequences, paste(dest.dir, "/", baseName, "_sampling_method.csv", sep=""), na="", row.names = FALSE)
  }
  if(exists("Reference")){
